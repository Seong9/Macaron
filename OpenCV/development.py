...
def next_frame_find_poly_already_fitted(binary_warped_left, binary_warped_right, left_fit, right_fit, lower_offset, upper_offset, following_right_lane, following_left_lane, road_width, PIXEL_CONVERSION_RATE, error_loop, margin=50, is_plot=True, add_debug_image=True):
    ...
    if abs(mid_fit_deg_ave) > 10:
        ...
        roi_ind_left = nonzeroy_left > (roi_limit + stopx_base)
        roi_ind_right = nonzeroy_right > (roi_limit + stopx_base)
...
def linetracker():
    ...
    while True:  # @@@@@@@@@@@@@@@@@@@@영상출력루프 시작@@@@@@@@@@@@@@@@@@@@@@
        ...
        if error == 0:
            left_fit, right_fit, lc, rc, deviation, debug_images2, error, following_left_lane, following_right_lane, top_offset, mid_fit_deg_ave, roi_limit = next_frame_find_poly_already_fitted(binary_all_left_gray, binary_all_right_gray, left_fit, right_fit, lower_offset, upper_offset, following_right_lane, following_left_lane,
...
print('step_1')
cam = cv2.VideoCapture('k_city_in_cut.mp4')
...
target_distance = 13.5  # 11.25  # 목표 거리 (현재 목표 10m전방)
